...
class Preprocessor:
    ...
    def remove_unwanted_cols(self,data,return_unwanted_data=False):
        ...
        try:
            self.data = self.df.drop(['tracking_id', 'datetime','turbine_status', 'cloud_level'],axis=1)
    ...
    def encoding_turbine(self,data):
        ...
        try:
            ...
            self.data['turbine_status_encoding'] =self.encoder.fit_transform(self.data['turbine_status'],self.data['wind_speed(m/s)'])
...
class PreProcessorRow:
    ...
    def remove_unwanted_cols(self,data,return_unwanted_data=False):
        ...
        try:
            self.data = self.df.drop(['datetime','turbine_status', 'cloud_level'],axis=1)

            if return_unwanted_data == True:
                self.unwanted_data = self.df[['datetime', 'wind_speed(m/s)','atmospheric_temperature(°C)', 'shaft_temperature(°C)','blades_angle(°)', 'gearbox_temperature(°C)', 'engine_temperature(°C)','motor_torque(N-m)', 'generator_temperature(°C)','atmospheric_pressure(Pascal)', 'area_temperature(°C)','windmill_body_temperature(°C)', 'wind_direction(°)', 'resistance(ohm)','rotor_torque(N-m)', 'turbine_status', 'cloud_level', 'blade_length(m)','blade_breadth(m)', 'windmill_height(m)']]
